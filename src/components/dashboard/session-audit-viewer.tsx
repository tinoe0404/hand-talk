...
export function SessionAuditViewer({ session }: SessionSummaryProps) {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            const dateStr = format(new Date(session.createdAt), "yyyyMMdd");
            pdf.save(`HandTalk_Audit_${session.patient.mrn}_${dateStr}.pdf`);
        } catch (error) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-zinc-900 tracking-tight">Clinical Session Record</h2>
                    <p className="text-sm font-medium text-zinc-500">
                        Official timeline log for MRN: {session.patient.mrn}
                    </p>
                </div>
                <button
                    onClick={handleExportPDF}
                    disabled={isExporting}
                    className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl font-bold text-sm shadow-md transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            {/* Print Wrapper */}
            <div ref={printRef} className="space-y-8 bg-clinical-offwhite p-2">
